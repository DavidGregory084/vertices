...
// echoMessagesExuberantly: monix.eval.Task[Unit] = Task.Async$173468983

// Kick that off in the background
echoMessagesExuberantly.runAsync(scheduler)
// res4: monix.execution.CancelableFuture[Unit] = Async(Future(<not completed>),monix.execution.cancelables.StackedCancelable$Impl@135292a)

// Send a message to the handler
val sendAMessage = vertx.eventBus.
  send[String]("echo", "hello").
  foreachL(msg => println(msg.body))
// sendAMessage: monix.eval.Task[Unit] = Task.Map$550126747
...
// res8: monix.execution.CancelableFuture[Unit] = Async(Future(<not completed>),monix.execution.cancelables.StackedCancelable$Impl@17dea716)
```
