...
case class HttpServerResponse(val unwrap: JavaHttpServerResponse) extends AnyVal {
  ...
  // Wrapper method
  def push(method: HttpMethod, host: String, path: String, handler: Handler[AsyncResult[HttpServerResponse]]): HttpServerResponse =
    HttpServerResponse(unwrap.push(method, host, path, ()))

  // Wrapper method
  def push(method: HttpMethod, path: String, headers: MultiMap, handler: Handler[AsyncResult[HttpServerResponse]]): HttpServerResponse =
    HttpServerResponse(unwrap.push(method, path, headers, ()))
